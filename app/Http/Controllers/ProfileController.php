...
class ProfileController extends Controller
{
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Send validation errors to SweetAlert
    ...
    public function allProfiles(Request $request)
    {
        ...
        // FILTER: Looking For (Male/Female)
        if ($request->looking_for) {
            $profiles->where('gender', strtolower($request->looking_for));
        }

        // FILTER: Age group (18-30)
        if ($request->age) {
            [$minAge, $maxAge] = explode('-', $request->age);

            $profiles->whereRaw("TIMESTAMPDIFF(YEAR, dob, CURDATE()) BETWEEN ? AND ?", [
                (int)$minAge,
                (int)$maxAge
            ]);
        }
        ...
        // FILTER: Religion
        if ($request->religion && $request->religion != "Any") {
            $profiles->where('religion', $request->religion);
        }

        // FILTER: Location → use country_of_residence
        if ($request->city && $request->city != "") {
            $profiles->where('country_of_residence', $request->city);
        }
